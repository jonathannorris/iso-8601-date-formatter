...
#endif
#import "TL_ISO8601DateFormatter.h"

#ifndef DEFAULT_TIME_SEPARATOR
#	define DEFAULT_TIME_SEPARATOR ':'
#endif
const unichar TL_ISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
@interface TL_ISO8601DateFormatter ()
...
@interface TL_ISO8601DateFormatter(UnparsingPrivate)
...
extern bool TL_ISO8601DateFormatter_GlobalCachesAreWarm(void);

bool TL_ISO8601DateFormatter_GlobalCachesAreWarm(void) {
	return (timeZonesByOffset != nil) && (timeZonesByOffset.count > 0);
}
#endif

@implementation TL_ISO8601DateFormatter
+ (void) initialize {
...
- (id) init {
	if ((self = [super init])) {
		...
		format = TL_ISO8601DateFormatCalendar;
		timeSeparator = TL_ISO8601DefaultTimeSeparatorCharacter;
		includeTime = NO;
		parsesStrictly = NO;
...
static NSUInteger TL_read_segment(const unichar *str, const unichar **next, NSUInteger *out_num_digits);
static NSUInteger TL_read_segment_4digits(const unichar *str, const unichar **next, NSUInteger *out_num_digits);
static NSUInteger TL_read_segment_2digits(const unichar *str, const unichar **next);
static double TL_read_double(const unichar *str, const unichar **next);
static BOOL TL_is_leap_year(NSUInteger year);
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange fractionOfSecond:(out NSTimeInterval *)outFractionOfSecond {
	...
	if (strict) timeSep = TL_ISO8601DefaultTimeSeparatorCharacter;
	...
	if (strict && isspace(*ch)) {
		...
		if (*ch == 'T') {
			...
			segment = TL_read_segment(ch, &ch, &num_digits);
			switch(num_digits) {
				case 0:
					if (*ch == 'W') {
								...
								switch(num_digits) {
									...
									case 2: //MM
										...
										if (!isdigit(*ch))
											...
											day = TL_read_segment(ch, &ch, NULL);
										break;

									case 3: //DDD
										day = segment % 1000U;
										dateSpecification = dateOnly;
										if (strict && (day > (365U + TL_is_leap_year(year))))
											isValidDate = NO;
										break;
						...
						case 0:
							if (*ch == '-') {
								...
								if (*++ch == 'W')
									...
									segment = TL_read_segment_4digits(ch, &ch, &num_digits);
									...
									switch(num_digits) {
										...
										case 2: //YY-MM; YY-MM-DD
											...
											if (*ch == '-') {
												if (isdigit(*++ch))
													day = TL_read_segment_2digits(ch, &ch);
												else
													day = 1U;
											} else
												day = 1U;
							...
							parseWeekAndDay: //*ch should be 'W' here.
								if (!isdigit(*++ch)) {
									...
									month_or_week = TL_read_segment_2digits(ch, &ch);
									if (*ch == '-') ++ch;
								parseDayAfterWeek:
									day = isdigit(*ch) ? TL_read_segment_2digits(ch, &ch) : 1U;
		...
		if (isValidDate) {
			...
			if (isdigit(*ch)) {
				hour = TL_read_segment_2digits(ch, &ch);
				if (*ch == timeSep) {
					++ch;
					if ((timeSep == ',') || (timeSep == '.')) {
						//We can't do fractional minutes when '.' is the segment separator.
						//Only allow whole minutes and whole seconds.
						minute = TL_read_segment_2digits(ch, &ch);
						if (*ch == timeSep) {
							++ch;
							second = TL_read_segment_2digits(ch, &ch);
						}
					} else {
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case TL_ISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case TL_ISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case TL_ISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %tu, not calendar (%tu), week (%tu), or ordinal (%tu)", self.format, TL_ISO8601DateFormatCalendar, TL_ISO8601DateFormatWeek, TL_ISO8601DateFormatOrdinal];
			return nil;
	}
}
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	BOOL yearIsLeapYear = TL_is_leap_year(year);
	BOOL prevYearIsLeapYear = TL_is_leap_year(prevYear);
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = TL_ISO8601DefaultTimeSeparatorCharacter;
